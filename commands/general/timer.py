...
for message in pubsub.listen():
    if message["type"] == "message":
        ...
        send_message_to_redis(f'{time_name.capitalize()} Timer set for {time_in_minutes} minute(s) 🤖 !')

        # Half point if time is longer then 25 minutes
        if time_in_seconds > 1500:
            threading.Timer(time_in_seconds / 2, send_message_to_redis, [f'@{message_obj["author"]["mention"]} your {time_name.capitalize()} Timer has halfway done! 🚨']).start()
        
        # 2 min left on timer
        threading.Timer(time_in_seconds - 120, send_message_to_redis, [f'@{message_obj["author"]["mention"]} your {time_name.capitalize()} Timer has 2 minutes left! 🚨']).start()
        
        # Notify the chat that the timer is up
        threading.Timer(time_in_seconds, send_message_to_redis, [f'@{message_obj["author"]["mention"]} your {time_name.capitalize()} Timer is up! 🚨']).start()
